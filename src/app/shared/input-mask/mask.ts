...
type StringHashMap<T> = { [key: string]: T }
...
export function valueToFormat2(value: string, format: string, goingBack = false, prevValue?: string): string {
    const unmaskedValue = unmaskValue(value);
    const limitToLoop = unmaskedValue.length - 1;
    let countRegex = 0;
    let newValue = '';


    for (let i = 0; countRegex <= limitToLoop && newValue.length < format.length; i++) {
        ...
        if (!formatRegex) {
            ...
            if (formatRegex.test(unmaskedValue[countRegex])) {
                newValue += unmaskedValue[countRegex];
            }
